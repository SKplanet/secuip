			...
			ap_log_error(APLOG_MARK, APLOG_ERR, 0, r->server, "EXPIRE COMMAND(current_count == max_call_count) [%s]", redis_command);
				...
				if (reply == NULL) {
					...
					return block_response_code;
                                ...
                                ap_log_error(APLOG_MARK, APLOG_ERR, 0, r->server, "REPLY(%s): type[%d], integer[%d], len[%d], str[%s], elements[%d]", redis_command, reply->type, reply->integer, reply->len, (reply->str == NULL ? "NULL" : reply->str), reply->elements);
